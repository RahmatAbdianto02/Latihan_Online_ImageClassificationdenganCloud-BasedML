        ...
        currentImageUri?.let {uri ->
            ...
            lifecycleScope.launch {
                try {
                    ...
                    successResponse.data?.let { data -> // Perbaikan di sini
                        binding.resultTextView.text = if (data.isAboveThreshold == true) {
                            showToast(successResponse.message.toString())
                            String.format("%s dengan %.2f%%", data.result, data.confidenceScore)
                        } else {
                            showToast("Model berhasil diprediksi tetapi di bawah ambang batas.")
                            String.format("Silakan gunakan gambar yang benar karena skor kepercayaan adalah %.2f%%", data.confidenceScore)
                        }
                        shoLoading(false) // Pindahkan ke dalam blok let
                    }
